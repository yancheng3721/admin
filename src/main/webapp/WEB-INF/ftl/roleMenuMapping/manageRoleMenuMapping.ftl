...
$(function() {
	...
	var allFiledsStr = '[{"_CN_":"ID","_KEY_":"ID"},{"_CN_":"角色ID","_KEY_":"ROLE_ID","_NOT_NULL_":"1"},{"_CN_":"菜单ID","_KEY_":"MENU_ID","_NOT_NULL_":"1"},{"_CN_":"更新用户","_KEY_":"UPDATE_USER"},{"_CN_":"创建时间","_KEY_":"CREATE_TIME"},{"_CN_":"更新时间","_KEY_":"UPDATE_TIME"}]';
    ...
    function expand(obj){
    	if(obj.ID){
			$.ajax({
				url: '/admin/roleMenuMapping/expandRoleMenuMapping.do',
				type: 'POST',
				dataType: 'json',
				timeout: 5000,
				data:obj,
	...
	function doBtchDel(){
		...
		$.ajax({
				url: '/admin/roleMenuMapping/batchDeleteRoleMenuMapping.do',
				type: 'POST',
				dataType: 'json',
				timeout: 5000,
				data:{ids:ids},
	...
	function doSave(){
		...
		//alert(222);
		$.ajax({
				url: '/admin/roleMenuMapping/saveRoleMenuMapping.do',
				type: 'post',
				dataType: 'json',
				timeout: 5000,
				data:params,
	...
	function doDelete(obj){
		...
		$.ajax({
				url: '/admin/roleMenuMapping/deleteRoleMenuMapping.do',
				type: 'post',
				dataType: 'json',
				timeout: 5000,
				data:obj,
	...
	function doQuery(){
		...
		searchForm.attr("action","/admin/roleMenuMapping/queryRoleMenuMapping.do");
		searchForm.submit();
	}
	...
	function syncData(){
		...
		$.ajax({
				url: '/admin/roleMenuMapping/syncRoleMenuMapping.do',
				type: 'POST',
				dataType: 'json',
		...
		$('#sguploadForm').ajaxSubmit({
                ...
                success:function(data){
                	...
                	if (data.obj.message == "上传成功!") {
                	   window.location.href="/admin/roleMenuMapping/manageRoleMenuMapping.do";
                	}
                },
                error:function(xhr){
	...
	function exportCsv() {
		var searchForm = $("#searchForm");
		var tmp = searchForm.attr("action");
		searchForm.attr("action","/admin/roleMenuMapping/exportRoleMenuMapping.do");
		searchForm.submit();
...
<form method="post" action="" id="searchForm">			
		<div class="mRtBox">
			...
			<div class="rTitle" >
				<h3><i class="title"></i><span id="title">角色菜单权限管理</span></h3>
			...
			<div class="searchCenter">
				<input type="hidden" name="exportFields" value="ROLE_NAME,MENU_NAME"/>
			...
			<div class="searchCenter pagenation" style="text-align:right">
				<script type="text/javascript">
					window.__page=new SnPage(document.getElementById("searchForm"),'/admin/roleMenuMapping/queryRoleMenuMapping.do','${page.totalPage?c}','${page.currentPage?c}');
				</script>
...
<div id="saveDialog" title="角色菜单权限保存" style="display:none" >
...
<td> <input type='text' class='savebox'  name='saveboxROLE_ID' id='saveboxROLE_ID' ></td>
</tr>
<tr style='height:35px;'>
<td width='140px' align='right'>菜单ID：</td>
<td> <input type='text' class='savebox'  name='saveboxMENU_ID' id='saveboxMENU_ID' ></td>
</tr>
...
<#--批量上传-->
<div class="showBox" id="batchupload" style="display:none;" title="角色菜单权限批量上传">
	<form action="/admin/roleMenuMapping/uploadRoleMenuMapping.do" method="POST" id="sguploadForm" name="sguploadForm" enctype="multipart/form-data" onsubmit="return false;">
		<div style="text-align: center;margin: 10px 0;">
			<span>路径：</span>
			<input type="hidden" name="importFields" value="ROLE_ID,MENU_ID"/>
