...
$(function() {
	...
	var allFiledsStr = '[{"_CN_":"ID","_KEY_":"ID"},{"_CN_":"模块ID","_KEY_":"MODULE_ID","_NOT_NULL_":"1"},{"_CN_":"角色ID","_KEY_":"ROLE_ID","_NOT_NULL_":"1"},{"_CN_":"更新用户","_KEY_":"UPDATE_USER"},{"_CN_":"创建时间","_KEY_":"CREATE_TIME"},{"_CN_":"更新时间","_KEY_":"UPDATE_TIME"}]';
    ...
    function expand(obj){
    	if(obj.ID){
			$.ajax({
				url: '/admin/moduleRoleMapping/expandModuleRoleMapping.do',
				type: 'POST',
				dataType: 'json',
				timeout: 5000,
				data:obj,
	...
	function doBtchDel(){
		...
		$.ajax({
				url: '/admin/moduleRoleMapping/batchDeleteModuleRoleMapping.do',
				type: 'POST',
				dataType: 'json',
				timeout: 5000,
				data:{ids:ids},
	...
	function doSave(){
		...
		//alert(222);
		$.ajax({
				url: '/admin/moduleRoleMapping/saveModuleRoleMapping.do',
				type: 'post',
				dataType: 'json',
				timeout: 5000,
				data:params,
	...
	function doDelete(obj){
		...
		$.ajax({
				url: '/admin/moduleRoleMapping/deleteModuleRoleMapping.do',
				type: 'post',
				dataType: 'json',
				timeout: 5000,
				data:obj,
	...
	function doQuery(){
		...
		searchForm.attr("action","/admin/moduleRoleMapping/queryModuleRoleMapping.do");
		searchForm.submit();
	}
	...
	function syncData(){
		...
		$.ajax({
				url: '/admin/moduleRoleMapping/syncModuleRoleMapping.do',
				type: 'POST',
				dataType: 'json',
		...
		$('#sguploadForm').ajaxSubmit({
                ...
                success:function(data){
                	...
                	if (data.obj.message == "上传成功!") {
                	   window.location.href="/admin/moduleRoleMapping/manageModuleRoleMapping.do";
                	}
                },
                error:function(xhr){
	...
	function exportCsv() {
		var searchForm = $("#searchForm");
		var tmp = searchForm.attr("action");
		searchForm.attr("action","/admin/moduleRoleMapping/exportModuleRoleMapping.do");
		searchForm.submit();
...
<form method="post" action="" id="searchForm">			
		<div class="mRtBox">
			...
			<div class="rTitle" >
				<h3><i class="title"></i><span id="title">模块角色对应关系管理</span></h3>
			...
			<div class="searchCenter">
				<input type="hidden" name="exportFields" value="MODULE_NAME,ROLE_NAME"/>
			...
			<div class="searchCenter pagenation" style="text-align:right">
				<script type="text/javascript">
					window.__page=new SnPage(document.getElementById("searchForm"),'/admin/moduleRoleMapping/queryModuleRoleMapping.do','${page.totalPage?c}','${page.currentPage?c}');
				</script>
...
<div id="saveDialog" title="模块角色对应关系保存" style="display:none" >
	<input type="hidden" class="savebox" name="saveboxID" id="saveboxID" >
	<div class="box-content">
	<table style="margin-top:10px;font-size:12px;">
		
		<tr style='height:35px;'> 
<td width='140px' align='right'>模块ID：</td>
<td><select style='width:100%;' class='savebox'  name='saveboxMODULE_ID' id='saveboxMODULE_ID'>
	<option value=''>选择模块ID</option>
<#if MODULE_IDSelect??>
<#list MODULE_IDSelect?keys as key>
	<option value='${key}'>${MODULE_IDSelect[key]}</option>
</#list>
</#if>
	</select>
</td>
...
<#--批量上传-->
<div class="showBox" id="batchupload" style="display:none;" title="模块角色对应关系批量上传">
	<form action="/admin/moduleRoleMapping/uploadModuleRoleMapping.do" method="POST" id="sguploadForm" name="sguploadForm" enctype="multipart/form-data" onsubmit="return false;">
		<div style="text-align: center;margin: 10px 0;">
			<span>路径：</span>
			<input type="hidden" name="importFields" value="MODULE_ID,ROLE_ID"/>
