package com.concentrate.search.admin.util;
...
public class ReadFileUtil {
	...
	// 读取本地多个套餐商品文件
	public static <T> List<T> readMultiFile(String prefix,
			CityFileRowMapper<T> frm, Map<String, Integer> cityMap) {
		List<T> list = new ArrayList<T>();
		File file = new File("/opt/search/admin/goodsPackage/");
		String path = file.getAbsolutePath();
		try {
			File[] files = new File(path).listFiles();
			for (File f : files) {
				if (f.isFile() && f.getName().startsWith(prefix) && f.exists()
						&& f.getName().contains("file")) {// 以日期开头，包含“file”的文件
	...
	// 读取本地文件夹下多个文件(预约，预定，到店试)
	public static <T> List<T> readMultiFile(String prefix, String local,
			CityFileRowMapper<T> frm, Map<String, Integer> cityMap) {
		List<T> list = new ArrayList<T>();
		...
		try {
			File[] files = new File(path).listFiles();
			LOGGER.info("文件夹" + path + "包含" + files.length + "个文件");
			for (File f : files) {
				if (f.isFile() && f.getName().startsWith(prefix) && f.exists()
						&& f.getName().contains("file")) {// 以日期开头，包含“file”的文件
					if (f.getName().endsWith(".txt")) {
						...
						try {
							...
							reader = new BufferedReader(isr);
							String line;
							LOGGER.info("读取文件" + f.getAbsolutePath() + "开始");
							while ((line = reader.readLine()) != null) {
								T t = frm.mapRow(line, cityMap);
								if (t != null) {
									list.add(t);
							...
							LOGGER.info("读取文件" + f.getAbsolutePath() + "结束");
						} catch (Exception e) {
							LOGGER.error("读取文件异常", e);
						} finally {
							if (reader != null) {
